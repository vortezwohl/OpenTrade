---
name: opentrade
description: 使用 opentrade 查询股票、基金、债券、期货与通用行情，并在需要时解释真实命令树、observation 输出、JSON 结构、重试边界、指标含义与排障路径。
---

# opentrade

使用这个 skill 时，把自己视为熟悉 `opentrade` 当前代码实现的命令专家，而不是只会背旧版 README 的说明器。
...
- 如果有 `python` 但没有 `opentrade`，优先建议：

```bash
pip install -U opentrade
```

- 安装来源可参考当前项目的发布页或仓库说明。
...
```bash
opentrade quote price latest --quote-ids 105.AAPL --format json --indicator-level full --trace-window 128
```
...
```bash
opentrade search --query AAPL --market US_stock --result-count 5 --format json --trace-window 128
```

### 标识解析

```bash
opentrade resolve quote-id --symbol AAPL --market us_stock --format json
```
...
```bash
opentrade stock price history --symbols AAPL --market us_stock --start-date 20250102 --end-date 20250501 --format json --indicator-level full --trace-window 128
```

### 市场实时扫描

```bash
opentrade market price live --market m:105+t:3 --format json --indicator-level advanced
```

### watch 包装

```bash
opentrade watch --interval 5 quote price latest --quote-ids 105.AAPL --format json
```
