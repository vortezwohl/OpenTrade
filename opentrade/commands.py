...
provider-specific 扩展命令也不再以 provider 名称直接暴露为顶层根组，
而是按自身声明的业务语义路径挂到统一命令树中。
"""
...
from opentrade.executor import (
    CommandExecutor,
    default_watch_count,
...
from opentrade.request_schema import build_click_options_for_schema
...
def _get_or_create_root_group(
    cli: click.Group, group_name: str
) -> click.Group:
    """按根组名获取或创建顶层命令分组。"""
...
def attach_definition_to_tree(
    root_group: click.Group, definition: CommandDefinition
) -> None:
    """把共享命令定义递归挂载到命令树。"""
    path_parts = definition.cli_path[1:]
    if not path_parts:
        raise ValueError(
            f"Invalid cli_path for shared command: {definition.cli_path}"
        )
    ...
    for part in path_parts[:-1]:
        ...
        if child is None:
            ...
            raise ValueError(
                f"Path conflict at {' '.join(definition.cli_path)}"
            )
        current_group = child
    current_group.add_command(
        create_shared_command(definition, command_name=path_parts[-1])
    )
...
def create_shared_command(
    ...
    def callback(**kwargs: Any) -> None:
        ...
        backend_selection = resolve_backend_selection(
            definition, runtime_kwargs["backend_name"]
        )
        ...
        request = InvocationRequest(
            ...
            watch=WatchOptions(
                ...
                count=default_watch_count(
                    runtime_kwargs["watch"], runtime_kwargs["count"]
                ),
...
def attach_runtime_options(
    command: click.Command, include_backend: bool = False
) -> None:
    """为命令统一追加运行时参数。"""
    ...
    if include_backend:
        params.append(
            click.Option(
                ...
                type=click.Choice(
                    [item.value for item in BackendName], case_sensitive=False
                ),
                default=None,
...
def build_shared_help_text(definition: CommandDefinition) -> str:
    ...
    if BackendName.YFINANCE in definition.supported_backends:
        lines.append(
            "注意: yfinance 以 Yahoo ticker / symbol 语义为主，本地市场专属命令面并不完整。"
        )
...
def create_watch_command() -> click.Command:
    ...
    @click.option(
        "--count", type=click.INT, default=None, help="刷新次数；不传时持续刷新。"
    )
    @click.option(
        "--clear/--no-clear",
        "clear_screen",
    ...
    def watch_command(
        ...
        if not ctx.args:
            raise click.ClickException(
                "watch must be followed by a full subcommand."
            )
        ...
        if (
            clear_screen
            and "--clear" not in forwarded
            and "--no-clear" not in forwarded
        ):
