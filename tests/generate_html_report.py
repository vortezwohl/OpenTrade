...
def render_result_section(result: dict, idx: int) -> str:
    """渲染单条测试结果。"""
    status_class = "ok" if result["success"] else (
        "warn" if result.get("allow_failure") else "danger"
    )
    status_text = "PASS" if result["success"] else (
        "FAIL*" if result.get("allow_failure") else "FAIL"
    )
    if result["exit_code"] == -1:
        status_text = "TIMEOUT"
        status_class = "warn"

    command_text = escape_html(result["command"])
    backend_text = f'{result["backend"]}/{result["format"]}'

    rows = [
        f'<div class="test-case" id="test-{idx}">',
        f'<div class="test-header {status_class}">',
        f'<span class="test-id">#{idx}</span>',
        f'<span class="test-cmd">efinance {command_text}</span>',
        f'<span class="test-status badge-{status_class}">{status_text}</span>',
        f'<span class="test-time">{result["elapsed_ms"]}ms</span>',
        f'<span class="test-backend">{backend_text}</span>',
        '</div>',
    ]

    if result.get("note"):
        rows.append(
            f'<div class="test-note">{escape_html(result["note"])}</div>'
        )

    if result.get("error"):
        rows.append(
            f'<div class="test-error">{escape_html(result["error"])}</div>'
        )
    ...
    if result.get("stderr"):
        ...
        rows.extend(
            [
                '<details class="test-stderr"><summary>stderr</summary>',
                f'<pre>{escape_html(stderr_text)}</pre>',
                '</details>',
            ]
        )
    ...
    if result.get("stdout"):
        rows.extend(
            [
                '<details class="test-stdout" open>',
                '<summary>真实 CLI 输出</summary>',
                f'<pre>{escape_html(result["stdout"])}</pre>',
                '</details>',
            ]
        )
...
<p>
  ...
  每条命令均通过真实网络请求调用 efinance / akshare / yfinance 后端。<br>
...
<p style="color:var(--muted)">
  点击每个测试项的「真实 CLI 输出」展开查看完整输出。<br>
  PASS=通过, FAIL=失败, FAIL*=允许失败, TIMEOUT=超时(90s)
</p>
'''
    ]
